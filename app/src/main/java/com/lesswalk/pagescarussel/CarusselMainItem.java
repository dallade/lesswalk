...
public class CarusselMainItem
{
    ...
    private CarusselGeneralItem  generalItem      = null;
    private CarusselParkingItem  parkingsItem[]   = null;
    private int                  parkingsItemSize = 0;
    private CarusselIndoorItem   indoorItem       = null;
    
    private File objectsDir = null;

    public CarusselMainItem(Context context)
    {
        this.context = context;
        //
        generalItem  = new CarusselGeneralItem(context);
        parkingsItem = new CarusselParkingItem[MAX_PARKING_SIZE];
...
//                    Log.d("elazarkin", "will init parkingItem[" + parkingsItemSize + "]");
                    parkingsItem[parkingsItemSize] = new CarusselParkingItem(context, parkingsItemSize);
                    parkingsItemSize++;
...
//                text = "GENERAL_MAPADDR_JSON_STATE";
            ...
            {
            	if(indoorItem == null) indoorItem = new CarusselIndoorItem(context);
            	indoorItem.initIndoorItem(objectsDir, key, value);
            }
            else if (checkJsonState(objArray, GENERAL_INDOORADDR_JSON_STATE, size))
            {
            	((CarusselGeneralItem)generalItem).initIndoorAddressItem(objectsDir, key, value);
            }
...
//                text = "GENERAL_JSON_STATE";
    ...
    public void loadJSON(String number)
    {
        ...
        try
        {
            ...
            String hashNum = "E68F1C41-656A-4DA8-A001-10093F93E000";
            objectsDir = new File(Environment.getExternalStorageDirectory().getPath()+File.separator + hashNum);
